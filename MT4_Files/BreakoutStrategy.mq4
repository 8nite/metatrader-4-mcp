...
int OnInit()
{
    ...
    gPointValue = Point;
    gTickSize = SymbolInfoDouble(Symbol(), SYMBOL_TRADE_TICK_SIZE);
    gDigits = (int)SymbolInfoInteger(Symbol(), SYMBOL_DIGITS);
    ...
    Print("Breakout Strategy EA initialized for ", Symbol());
...
{
    CleanUpVisuals();
    Print("Breakout Strategy EA deinitialized");
}
...
void UpdateTradingPermission()
{
    ...
    Comment("Breakout Strategy EA - Ready to trade");
}
...
double CalculateLotSize()
{
    ...
    double pipValue = tickValue / tickSize * gPointValue;
...
double FindNextSRLevel(bool isBuy, int currentLevelIndex)
{
    ...
    double bestLevel = 0;
    double minDistance = 1000 * gPointValue; // Minimum distance for valid level
