//+------------------------------------------------------------------+
//|                                 ProfessionalBreakoutEA.mq4 |
//|                        Copyright 2024, Professional Breakout EA |
//|                              Advanced Breakout Trading Strategy |
//+------------------------------------------------------------------+
#property copyright "Copyright 2024, Professional Breakout EA"
#property link      ""
...
input group "=== FILTERING SETTINGS ==="
...
int OnInit()
{
    ...
    Print("Professional Breakout EA initialized for ", Symbol());
...
{
    CleanUpVisuals();
    Print("Professional Breakout EA deinitialized");
}
...
void UpdateTradingPermission()
{
    ...
    Comment("Professional Breakout EA - Ready | S/R Levels: " + IntegerToString(gSRCount));
}
...
{
    gSRCount++;
    ArrayResize(gSRLevels, gSRCount);
    
    gSRLevels[gSRCount-1].price = price;
    gSRLevels[gSRCount-1].time = time;
    gSRLevels[gSRCount-1].touches = touches;
    gSRLevels[gSRCount-1].isSupport = isSupport;
}
...
void SortSRLevelsByStrength()
{
    for (int i = 0; i < gSRCount - 1; i++) {
        for (int j = 0; j < gSRCount - 1 - i; j++) {
            if (gSRLevels[j].touches < gSRLevels[j+1].touches) {
                SRLevel temp = gSRLevels[j];
                gSRLevels[j] = gSRLevels[j+1];
                gSRLevels[j+1] = temp;
            }
        }
    }
...
void DrawSupportResistanceLevels()
{
    ...
    // Draw current levels
    for (int i = 0; i < gSRCount && i < 8; i++) {
        string objName = "SR_" + IntegerToString(i);
        color lineColor = gSRLevels[i].isSupport ? SupportColor : ResistanceColor;
        
        ObjectCreate(objName, OBJ_HLINE, 0, 0, gSRLevels[i].price);
        ...
        ObjectCreate(labelName, OBJ_TEXT, 0, Time[1], gSRLevels[i].price);
        ObjectSetText(labelName, "T:" + IntegerToString(gSRLevels[i].touches), 8, "Arial", lineColor);
    }
...
void CheckBreakoutSignals()
{
    ...
    for (int i = 0; i < gSRCount; i++) {
        double level = gSRLevels[i].price;
        bool isSupport = gSRLevels[i].isSupport;
...
void ExecuteBreakoutTrade(bool isBuy, double breakoutLevel, int levelIndex)
{
    ...
    int ticket = -1;
    string comment = "Breakout_" + (isBuy ? "Buy" : "Sell");
    ...
    if (ticket > 0) {
        Print("Breakout trade: ", (isBuy ? "BUY" : "SELL"), " ", lotSize, " lots");
    }
}
...
{
    double currentLevel = gSRLevels[currentLevelIndex].price;
    double bestLevel = 0;
    double minDistance = 100 * gPointValue;
